...
noWander.newActivity = (req, res, next) => {
  ...
  db
    .one(
      "INSERT INTO activity(name, description, price, votes, itinerary_id) VALUES($1, $2, $3, $4, $5) RETURNING id",
      [
        req.body.name,
...
noWander.deleteItinerary = (req, res, next) => {
  db.none("DELETE FROM itinerary where id = $1", [req.params.id]).then(() => {
    next();
  });
};
// ----------------------------------------------------
...
noWander.deleteActivity = (req, res, next) => {
  db.none("DELETE FROM activity where id = $1", [req.params.id]).then(() => {
    next();
  });
};
// ----------------------------------------------------
